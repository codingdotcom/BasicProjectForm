...
import {AppRegistry, Animated, Easing, Image, Text, View, StyleSheet, TouchableOpacity, ScrollView} from 'react-native';

const categoryLinks = [
  //for the sake of simplicity, we use the same set of category links for all touts
  {label: 'Subcategory1'},
  {label: 'Subcategory2'},
  {label: 'Subcategory3'},
  {label: 'Subcategory4'},
...
const categoryTouts = [
  //the touts are the clickable image items that hold our links
...
class CategoryLinks extends React.Component {
  //using PureComponent will prevent unnecessary renders
  ...
  render() {
    return (
      <Animated.View //view should be animated because its opacity will change
        style={{backgroundColor: 'orange', position: 'absolute', top: 0, left: 0}}>
        <View>
          {this.props.links &&
            this.props.links.map((link) => {
              //render our subcategory links
              return (
                <View key={link.label}>
...
class Tout extends React.Component {
  ...
  measureToutRef = () => {
    this.toutRef.measure((x, y, width, height, pageX, pageY) => {
      //measuring gives us all of these properties, so we must capture them and pass down only the two we need
  ...
  handlePressTout = () => {
    if (this.props.links && this.props.links.length) {
      //if the tout has subcategory links, hide or show them based on the current state
  ...
  showToutSubcatgories = (pageY) => {
    ...
    Animated.timing(this.animatedValue, {
      //animating this value from zero to one will update the subcategory container height, which interpolates this value
      toValue: 1,
  ...
  render() {
    let categoryLinks;
    if (this.props.links && this.props.links.length) {
      //if the tout has links, render them here
      categoryLinks = (
        <Animated.View style={{height: this.animCategoryHeight}}>
    ...
    return (
      ...
      >
        <TouchableOpacity ref={this.setToutRef} onPress={this.handlePressTout}>
          <Image source={{uri: this.props.image}} style={styles.toutImage} width={'100%'} height={206} resizeMode="cover" />
          <View style={{flex: 1}}>
            <Text
...
export default class scrollAccordion extends React.Component {
  //scroll accordion is our parent class - it renders the touts and their subcategories

  measurements = [];

  handlePressTout = (toutIndex) => {
    //when we press a tout, animate it to the top of the screen and reveal its subcategoires
